...
async def main_롱_한계치도달(): #실행시킬 함수명 임의지정
    while True:
        try:
            bot = telegram.Bot(token)
            await bot.send_message(chat_id, f"롱 물량 한계치 도달로 {롱_손절_물량}익절 반납\n익절 = {count_익절}")
            break
        except:
            await asyncio.sleep(timesleep)
            continue        
        ...
        if 롱_청산_주문갯수 >= 손절_실행_계약수 :
            ...
            while True :
                try:
                    ...
                    if 미체결주문:
                        for order in 미체결주문:
                            if order['info']['positionSide'] == 'LONG' and order['info']['side'] == 'SELL':
                                try:
