...
def get_previous_hour_candles(symbol, count=200, telegram_token=None, chat_id=None):
    try:
        ...
        target_candle = all_data[1]
        target_time = datetime.strptime(target_candle['candle_date_time_kst'], "%Y-%m-%dT%H:%M:%S")
        current_time = datetime.now()

        if target_time != (current_time - timedelta(hours=1)).replace(minute=0, second=0, microsecond=0):
            return None, [], 0, 0, 0, True

        previous_candles = all_data[2:12]
        ...
        return target_volume, previous_volumes, average_volume, volume_threshold, target_change, False
    except Exception as e:
...
async def main():
    ...
    while True:
        # Exclude invalid symbols from trading_list
        common_symbols -= set(invalid_symbols)
        invalid_symbols_next = []
        ...
        for symbol in tqdm(sorted(common_symbols), desc="심볼 처리 중"):
            target_volume, previous_volumes, average_volume, volume_threshold, target_change, is_invalid = get_previous_hour_candles(symbol, telegram_token=telegram_token, chat_id=chat_id)
            if is_invalid:
                invalid_symbols.append(symbol)
            elif target_volume and len(previous_volumes) == 10:
        ...
        # Proceed with trading for valid symbols first
        ...
        await log_and_notify(f"시간 불일치 코인: {invalid_symbols}", telegram_token, chat_id)

        # Process invalid symbols for trading
        while invalid_symbols:
            for symbol in invalid_symbols[:]:
                target_volume, previous_volumes, average_volume, volume_threshold, target_change, is_invalid = get_previous_hour_candles(symbol, telegram_token=telegram_token, chat_id=chat_id)
                if not is_invalid and target_volume >= volume_threshold and target_change > 0:
                    trading_list.append(symbol)
                else:
                    invalid_symbols_next.append(symbol)
                await asyncio.sleep(0.2)
            invalid_symbols = invalid_symbols_next
            invalid_symbols_next = []

        await log_and_notify(f"추가 검증 후 매매 대상 코인: {trading_list}", telegram_token, chat_id)
