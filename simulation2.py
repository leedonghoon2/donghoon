...
async def main():
    ...
    # 원하는 타임프레임 설정 (여기서는 '1h'로 설정하여 매 정각에 거래하도록 함)
    candle_timeframe = '1h'
    ...
    # 예를 들어 "2023-12-31 14:00:00"과 같이 설정할 수 있습니다.
    ...
    # 최초 사이클 기준 시작 시각을 현재 시각의 정각으로 맞춤
    now = datetime.now()
    ...
    while True:
        # 사이클 시작 시각은 실제 작업 시작 시각(datetime.now())으로 로그에 남깁니다.
