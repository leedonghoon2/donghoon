...
# 아래 순서로 텔레그램 알림을 전송한 후 주문을 재시도합니다.
async def open_short_position(exchange, symbol, margin_usd, open_positions, semaphore, telegram_token, chat_id):
    ...
    try:
        ...
        if '"code":-2027' in err_str:
            try:
                brackets = await exchange.fetch_leverage_brackets()
                symbol_brackets = brackets.get(symbol, [])
                new_leverage = None
                if symbol_brackets:
                    new_leverage = symbol_brackets[0]['initialLeverage']
                # 최대 허용 레버리지가 20 이상이거나 값을 못가져왔으면 업데이트하지 않음.
                ...
                await exchange.set_leverage(new_leverage, symbol)
